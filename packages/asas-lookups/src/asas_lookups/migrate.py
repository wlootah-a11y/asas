...
def _assert_adoptable(inspector) -> None:
    ...
    if _RENAMED_PAIRS and all(
        ...
        raise RuntimeError(
            f"asas-lookups found its own post-rename {_SENTINEL_TABLE!r} schema but no "
    ...
    if renamed and len(renamed) < len(_RENAMED_PAIRS):
        ...
        raise RuntimeError(
            f"asas-lookups cannot adopt the existing {{_SENTINEL_TABLE!r}} table: its columns are "
